...
- The output CSV file from total_cell_area.ijm. This is essentially the input CSV/Excel file used for total_cell_area.ijm, but with a column containing the total cell areas per image. This is possible since the videos and images are recorded on the same location of the chip.
- Folders of image-time sequences generated from mp4 videos using FFmpeg. The videos (and thus folders) have the same name as the corresponding images.
